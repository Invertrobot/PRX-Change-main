...
from hard_feature_refine.sampling_points import point_sample
from utils.prx_change_model import *
from dataloader.change_dataloader import get_loader
from visualize.process_bar import *
...
def train(train_loader, model, optimizer, epoch):
    ...
    loss_sum, avg_loss, process_step, cls_loss_sum, ref_loss_sum = 0, 0, 0, 0, 0
    ...
    for i, pack in enumerate(train_loader, start=1):
        ...
        if opt.mixed_precision_training:
            ...
            """calc class loss"""
            cls_loss = BCE(pred, gts)

            """calc refinement loss"""
            ref_loss = 0.0
            points = results['refines']
            for k in range(len(points)):
                gt_points = point_sample(
                    gts.float(),
                    points[k]["hard_points"],
                    mode="nearest",
                    align_corners=False
                )
                ref_loss += BCE(points[k]["RP"], gt_points)
            ref_loss /= len(points)

            """calc final loss"""
            loss = float(opt.l1) * cls_loss + float(opt.l2) * ref_loss
        ...
        cls_loss_sum += cls_loss.cpu().item()
        avg_cls_loss = cls_loss_sum / i

        ref_loss_sum += ref_loss.cpu().item()
        avg_ref_loss = ref_loss_sum / i
        ...
        process_bar_refine(process_step, epoch, avg_loss, step_loss, avg_cls_loss, avg_ref_loss,
                           optimizer.param_groups[0]['lr'], time_str)
        '''Training progress visualize'''
...
if __name__ == '__main__':
    ...
    opt.save_path = opt.save_path + '{}_{}_refine_{}_cross_{}/'.format(
        datetime.now(), opt.best_name, opt.refine, opt.cross
    )
