...
class WindowCrossAttention(nn.Module):
    ...
    def forward(self, feat1, feat2):
        B, N, C = feat1.shape
        qkv1 = self.qkv(feat1).reshape(B, -1, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q1, k1, v1 = qkv1[0], qkv1[1], qkv1[2]

        qkv2 = self.qkv(feat2).reshape(B, -1, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q2, k2, v2 = qkv2[0], qkv2[1], qkv2[2]

        attn_12 = (q1 @ k2.transpose(-2, -1)) * self.scale
        attn_12 = self.pos_emb_funct(attn_12, self.resolution ** 2)

        attn_12 = attn_12.softmax(dim=-1)
        attn_12 = self.attn_drop(attn_12)
        context_12 = (attn_12 @ v2).transpose(1, 2).reshape(B, -1, C)
        context_12 = self.proj(context_12)
        context_12 = self.proj_drop(context_12)

        attn_21 = (q2 @ k1.transpose(-2, -1)) * self.scale

        attn_21 = self.pos_emb_funct(attn_21, self.resolution ** 2)

        attn_21 = attn_21.softmax(dim=-1)
        attn_21 = self.attn_drop(attn_21)
        context_21 = (attn_21 @ v1).transpose(1, 2).reshape(B, -1, C)
        context_21 = self.proj(context_21)
        context_21 = self.proj_drop(context_21)
        return context_21, context_12
...
class HAT(nn.Module):
    ...
    def __init__(self,
        ...
        if sr_ratio > 1:
            ...
            self.hat_attn = WindowCrossAttention(
                dim,
    ...
    def forward(self, inputs):
        ...
        feat2, ct2 = inputs[1]
        B, T, N = feat1.shape
        feat1 = self.pos_embed(feat1)
        feat2 = self.pos_embed(feat2)
        ...
        if self.sr_ratio > 1:
            ...
            E = int(np.sqrt(ct1.shape[1]))
            # ct1 = ct_dewindow(ct1, self.cr_window * self.sr_ratio, self.cr_window * self.sr_ratio, self.cr_window)
            # ct2 = ct_dewindow(ct2, self.cr_window * self.sr_ratio, self.cr_window * self.sr_ratio, self.cr_window)
            ct1 = ct_dewindow(ct1, E, E, self.cr_window)
            ct2 = ct_dewindow(ct2, E, E, self.cr_window)

            # positional bias for carrier tokens
            ct1 = self.hat_pos_embed(ct1)
            ct2 = self.hat_pos_embed(ct2)

            # attention plus mlp
            ct1, ct2 = self.hat_attn(self.hat_norm1(ct1), self.hat_norm1(ct2))
            ct1 = ct1 + self.hat_drop_path(self.gamma1 * ct1)
            ct2 = ct2 + self.hat_drop_path(self.gamma1 * ct2)
            ct1 = ct1 + self.hat_drop_path(self.gamma2 * self.hat_mlp(self.hat_norm2(ct1)))
            ct2 = ct2 + self.hat_drop_path(self.gamma2 * self.hat_mlp(self.hat_norm2(ct2)))

            # ct are put back to windows
            ct1 = ct_window(ct1, E, E, self.cr_window)
            ct2 = ct_window(ct2, E, E, self.cr_window)

            ct1 = ct1.reshape(feat1.shape[0], -1, N)
            ct2 = ct2.reshape(feat2.shape[0], -1, N)
            # concatenate carrier_tokens to the windowed tokens
            feat1 = torch.cat((ct1, feat1), dim=1)
            feat2 = torch.cat((ct2, feat2), dim=1)

        # window attention together with carrier tokens
        feat1, feat2 = self.attn(self.norm1(feat1), self.norm2(feat2))
        feat1 = feat1 + self.drop_path(self.gamma3 * feat1)
        feat2 = feat2 + self.drop_path(self.gamma3 * feat2)
        feat1 = feat1 + self.drop_path(self.gamma4 * self.mlp(self.norm2(feat1)))
        feat2 = feat2 + self.drop_path(self.gamma4 * self.mlp(self.norm2(feat2)))

        if self.sr_ratio > 1:
            # for hierarchical attention we need to split carrier tokens and window tokens back
            ctr1, feat1 = feat1.split(
                [feat1.shape[1] - self.window_size * self.window_size, self.window_size * self.window_size],
                dim=1)
            ct1 = ctr1.reshape(Bg, Ng, Hg)  # reshape carrier tokens.

            ctr2, feat2 = feat2.split(
                [feat2.shape[1] - self.window_size * self.window_size, self.window_size * self.window_size],
                dim=1)
            ct2 = ctr2.reshape(Bg, Ng, Hg)  # reshape carrier tokens.
            if self.last and self.do_propagation:
                # propagate carrier token information into the image
                ctr_image_space1 = ctr1.transpose(1, 2).reshape(B, N, self.cr_window, self.cr_window)
                feat1 = feat1 + self.gamma1 * self.upsampler(ctr_image_space1.to(dtype=torch.float32)).flatten(
                    2).transpose(1, 2).to(dtype=feat1.dtype)

                ctr_image_space2 = ctr2.transpose(1, 2).reshape(B, N, self.cr_window, self.cr_window)
                feat2 = feat2 + self.gamma1 * self.upsampler(ctr_image_space2.to(dtype=torch.float32)).flatten(
                    2).transpose(1, 2).to(dtype=feat2.dtype)
        return [[feat1, ct1], [feat2, ct2]]
...
class FasterViTLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.window_size = window_size
        self.down = nn.Conv2d(dim,dim, kernel_size=patch_size, stride=patch_size)
        self.upsample = nn.ConvTranspose2d(dim, dim, kernel_size=patch_size, stride=patch_size)
    ...
    def forward(self, feat1, feat2):
        residual1 = feat1
        residual2 = feat2
        feat1 = self.down(feat1)
        feat2 = self.down(feat2)

        align_feat = torch.cat([feat1, feat2], dim=0)
        feat1, feat2 = torch.chunk(self.norm1(align_feat), 2, dim=0)

        ct1 = self.global_tokenizer(feat1) if self.do_gt else None
        ct2 = self.global_tokenizer(feat2) if self.do_gt else None
        B, C, H, W = feat1.shape
        if self.transformer_block:
            feat1 = window_partition(feat1, self.window_size)
            feat2 = window_partition(feat2, self.window_size)
        inputs = [[feat1, ct1], [feat2, ct2]]
        for bn, blk in enumerate(self.blocks):
            output = blk(inputs)
            feat1 = output[0][0]
            feat2 = output[1][0]
        if self.transformer_block:
            feat1 = window_reverse(feat1, self.window_size, H, W)
            feat2 = window_reverse(feat2, self.window_size, H, W)
        feat1 = self.upsample(feat1)
        feat2 = self.upsample(feat2)

        align_feat = torch.cat([feat1, feat2], dim=0)
        feat1, feat2 = torch.chunk(self.norm2(align_feat), 2, dim=0)

        return feat1 + residual1, feat2 + residual2


if __name__ == '__main__':
    fl = FasterViTLayer(dim=128, depth=1, input_resolution=32, num_heads=4, window_size=4)
    x1 = torch.zeros([4, 128, 64, 64])
    x2 = torch.zeros([4, 128, 64, 64])
    x1, x2 = fl(x1, x2)
